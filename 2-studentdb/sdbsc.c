...
int get_student(int fd, int id, student_t *s){
    if (id < MIN_STD_ID || id > MAX_STD_ID) { //verify valid id
        return ERR_DB_OP;
    }
    
    off_t offset = id * STUDENT_RECORD_SIZE; //find offset

    if (lseek(fd, offset, SEEK_SET) == -1) { //seek to the offset position
    ...
    if (memcmp(s, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE) == 0) { //check if the record is empty
...
int add_student(int fd, int id, char *fname, char *lname, int gpa){
    if (validate_range(id, gpa) == EXIT_FAIL_ARGS) { //validate student args
        printf(M_ERR_STD_RNG); 
        return ERR_DB_OP;        
    }

    student_t student = {0}; //intialize student
    int offset = id * STUDENT_RECORD_SIZE;

    if (lseek(fd, offset, 0) == -1) { //seek to offset position
        return ERR_DB_FILE;
    }

    ssize_t bytes_read = read(fd, &student, STUDENT_RECORD_SIZE); //read the db file
    if (bytes_read == -1) {
        printf(M_ERR_DB_READ);
        return ERR_DB_FILE;
    }

    if (memcmp(&student, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE) != 0) { //checks for duplicate student id
        printf(M_ERR_DB_ADD_DUP, id);
        return ERR_DB_OP;
    }

    strncpy(student.fname, fname, sizeof(student.fname) - 1); //new student info prep
    strncpy(student.lname, lname, sizeof(student.lname) - 1);
    student.id = id;
    student.gpa = gpa;


    if (lseek(fd, offset, SEEK_SET) == -1){ //seek to the correct position to write into db
        printf(M_ERR_DB_WRITE);
        return ERR_DB_FILE;
    }

    if (write(fd, &student, STUDENT_RECORD_SIZE) != STUDENT_RECORD_SIZE){ //write new student into db file
        printf(M_ERR_DB_WRITE);
        return ERR_DB_FILE;
    }
...
int del_student(int fd, int id){
    if (id < MIN_STD_ID || id > MAX_STD_ID) { //validate student id
        return ERR_DB_OP;
    }

    student_t student = {0}; //initialize student
    int get_result = get_student(fd, id, &student); //locate student to be deleted

    if (get_result == SRCH_NOT_FOUND) { //student not found
        printf(M_STD_NOT_FND_MSG, id);  
        return ERR_DB_OP;
    }
    if (get_result == ERR_DB_FILE) { //db error
        printf(M_ERR_DB_READ); 
        return ERR_DB_FILE;
    }
    ...
    if (lseek(fd, offset, SEEK_SET) == -1) { //seek to offset position
        printf(M_ERR_DB_WRITE);
        return ERR_DB_FILE;
    }

    if (write(fd, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE) != STUDENT_RECORD_SIZE) { //write empty record to show that student has been deleted
        printf(M_ERR_DB_WRITE);
        return ERR_DB_FILE;
    }
...
int count_db_records(int fd){
    ...
    if (lseek(fd, 0, SEEK_SET) == -1) { //seek to the beginning of file
        printf(M_ERR_DB_READ);
        return ERR_DB_FILE;
    }

    while (read(fd, &student, STUDENT_RECORD_SIZE) == STUDENT_RECORD_SIZE) { //read records til EOF
        if (memcmp(&student, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE) != 0) { //check if slot is empty or previously deleted if not add to count
            count++;
        }
    }

    if (count == 0) { //check to print whether the db is empty or is populated
        printf(M_DB_EMPTY);
    } else {
...
int print_db(int fd){
    student_t student = {0}; //intialize student
    bool found = false; //find first valid record
    ...
    while (read(fd, &student, STUDENT_RECORD_SIZE) == STUDENT_RECORD_SIZE) { //read til EOF
        if (memcmp(&student, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE) != 0) { //checks if record is empty
            if (!found) {
                printf(STUDENT_PRINT_HDR_STRING, "ID", "FIRST NAME", "LAST_NAME", "GPA"); //print the header after first real row encountered
                found = true;
            }
            float calculatedGPA = student.gpa / 100.0;
            printf(STUDENT_PRINT_FMT_STRING, student.id, student.fname, student.lname, calculatedGPA); //print student info
        }
    }

    if (!found) { //if there were no records, print the empty db message
        printf(M_DB_EMPTY);
    }
...
void print_student(student_t *s){
    if (s == NULL) { //check if student pointer is valid
        printf(M_ERR_STD_PRINT);
        return;
    }

    if (s->id == 0) {  //check if student pointer is valid
        printf(M_ERR_STD_PRINT);
        return;
    }

    float calculatedGPA = s->gpa / 100.0;
    printf(STUDENT_PRINT_HDR_STRING, "ID", "FIRST NAME", "LAST_NAME", "GPA"); //print header + student info
